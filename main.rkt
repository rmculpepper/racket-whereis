...
;; whereis-auto : String -> Path
(define (whereis-auto thing)
  (define-syntax-rule (handle e) (with-handlers ([exn? (lambda (e) #f)]) e))
  (define cache (make-hash))
  (define (score-path p)
    (define pkg (path->pkg p #:cache cache))
    ;; Prefer non-pkg, then -lib pkg, then other.
    (cond [(not pkg) -1] [(regexp-match? #rx".*-lib$" pkg) 0] [else 1]))
  (path->directory-path
   (or (handle (path-only (whereis-module `(lib ,thing))))
       (handle (car (sort (whereis-collection thing) < #:key score-path #:cache-keys? #t)))
       (handle (whereis-pkg thing))
       (error 'whereis-auto "failed to find any path for ~e:" thing))))
